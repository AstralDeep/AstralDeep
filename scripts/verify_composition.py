...
def _plane_schema_literal_import(
    ...
    if (len(imports) != 1 or imports[0] not in tree.body or imports[0].level != 0
        ...
        raise CompositionError(f"Plane {stem} schema import is not the reviewed exact import")
    for node in ast.walk(tree):
        ...
        if ((isinstance(node, ast.Name) and node.id == symbol and isinstance(node.ctx, (ast.Store, ast.Del)))
                ...
                or (isinstance(node, (ast.Import, ast.ImportFrom)) and any(
                    ...
                    for alias in node.names))):
            raise CompositionError(f"Plane {stem} schema import is rebound or ambiguous")
    path = component_root / f"src/astralplane/database/{stem}_schema.py"
    try:
        ...
        raise CompositionError(f"Plane {stem} schema source is unavailable") from exc
    if redirected:
        raise CompositionError(f"Plane {stem} schema must be a regular local source file")
    try:
        if not path.is_file() or path.stat().st_size > 1_048_576:
            raise CompositionError(f"Plane {stem} schema source is missing or exceeds the byte bound")
    ...
    if (not 1 <= len(items) <= _MAX_LITERAL_SEQUENCE_ITEMS or any(
            ...
            for item in items)):
        raise CompositionError(f"Plane {stem} schema must be a bounded nonempty string tuple")
...
def _plane_migration_digest(component_root: Path) -> str:
    ...
    literals = _literal_assignments(path, reviewed_literals=reviewed_literals)
