"""Tests for orchestrator/model_router.py: cheap-first task-to-tier mapping,
device-capability gating for on-device eligibility, escalation and confidence
heuristics, and end-to-end route() resolution.
"""
...
def test_tier_for_task_hint_overrides():
    ...
    assert mr.tier_for_task("chat_title", hint=99) == SMALL
...
def test_can_use_ondevice_requires_capability_and_simple_task():
    ...
    assert mr.can_use_ondevice(caps, "chat_title") is True
    assert mr.can_use_ondevice(caps, "ui_designer") is False
...
def test_resolve_model_uses_tier_map():
    ...
    assert mr.resolve_model(MEDIUM, "d", tier_map=tm) == "d"
...
def test_route_floors_at_small_and_defaults_model():
    ...
    assert d.tier == SMALL and d.model == "def"
    assert d.tier >= SMALL
