...
def registration(monkeypatch):
    ...
    orch = SimpleNamespace(local_agents={}, register_agent=_register)
    return orch, attempted, registered
...
async def test_flag_off_remote_agent_never_loads_or_registers(monkeypatch, registration):
    ...
    orch, attempted, registered = registration
...
def test_flag_off_menu_has_no_remote_machines_item(monkeypatch):
    _set_flag(monkeypatch, False)
    assert "remote-machines" not in _menu_item_keys(build_menu_model(["user"]))


def test_flag_on_contrast_menu_carries_remote_machines_item(monkeypatch):
    _set_flag(monkeypatch, True)
    assert "remote-machines" in _menu_item_keys(build_menu_model(["user"]))
