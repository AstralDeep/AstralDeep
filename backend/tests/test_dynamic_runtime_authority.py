"""Tests for per-runtime LETS authority hand-off at spawn
(orchestrator/dynamic_runtime_authority.py, generated_lets_executor.py): child
environment derivation, per-runtime audience agreement, and private root isolation.
"""
...
async def test_enforce_refusal_after_roots_were_prepared_removes_them(
    ...
    monkeypatch.setenv("LETS_MODE", "enforce")
    config = _config(tmp_path)
    config.trust_manifest = SimpleNamespace(warden_id="bad warden")
    coordinator = _Coordinator(config, binding=_active_binding_for)
    manager, orchestrator, supervisor = _manager(tmp_path, coordinator, monkeypatch)
