"""Tests for orchestrator/taint.py: the trust lattice (effective trust = min over
ancestors), source/sink classification, the user-supplied exemption, TaintTracker's
flow policy, and taint surviving multi-hop laundering.
"""
...
def test_search_then_fetch_is_not_denied():
    ...
    assert t.trust_of(url) == UNTRUSTED
...
def test_laundering_through_an_internal_tool_survives():
    ...
    src_b = classify_source("util-1", "format_text")
    ...
    assert out_b == UNTRUSTED
    assert t.trust_of("laundered-output") == UNTRUSTED
