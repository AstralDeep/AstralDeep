...
def _management_context(orch) -> PlaneRepositoryContext:
    source = plane_source_from_orchestrator(orch)
    repository, runtime = repository_from(
        "agent_management",
        plane_runtime=getattr(source, "plane_runtime", None),
...
async def _list_context(orch, user_id):
    ...
    return (
        _email_fallback(record.email, user_id),
        ownership_map,
        set(record.disabled_agent_ids),
    )
...
def _render_external_identity(external_identity_links, agent_id: str, card) -> str:
    metadata = getattr(card, "metadata", None) or {}
    ...
    linked = next(
        (
            link
            for link in external_identity_links
            if link.agent_id == agent_id and link.provider == provider
        ),
        None,
    )
    if linked:
...
async def _detail_context(orch, user_id, agent_id) -> dict:
    ...
    scope_state = {kind: False for kind in PERMISSION_KINDS}
    for state in record.scope_states:
        if state.scope in scope_state:
            scope_state[state.scope] = state.enabled
    return {
        "user_email": _email_fallback(record.email, user_id),
        "owner_email": None if ownership is None else ownership.owner_email,
        "is_public": bool(ownership is not None and ownership.is_public),
        "is_safe": record.is_safe,
        "safe_known": record.safe_known,
        "enabled": not record.disabled,
        "credential_keys": list(record.credential_keys),
        "scope_state": scope_state,
        "scope_states": record.scope_states,
        "tool_override_states": record.tool_override_states,
        "external_identity_links": record.external_identity_links,
    }
...
async def handle_visibility_set(orch, websocket, user_id, roles, payload):
    ...
    if not context["owner_email"]:
        return ("agents", params, notice_block(
            "error", f"No ownership record for agent '{agent_id}'."
        ))
    user_email = context["user_email"]
    if not user_email or context["owner_email"] != user_email:
        return ("agents", params, notice_block(
            "error", "Only the agent owner can change visibility."
        ))
    is_public = bool(payload.get("is_public"))
    changed = await asyncio.to_thread(
        orch.user_agent_registry.set_agent_visibility,
        agent_id,
        is_public,
    )
    if not changed:
...
async def handle_safe_set(orch, websocket, user_id, roles, payload):
    ...
    context = await _detail_context(orch, user_id, agent_id)
    user_email = context["user_email"]
    if user_email and context["owner_email"] == user_email:
...
async def handle_agent_enabled(orch, websocket, user_id, roles, payload):
    ...
    await asyncio.to_thread(
        orch.tool_permissions.set_agent_disabled, user_id, agent_id, not enabled)
    logger.info(
