"""BYO agent authoring: create -> analyze -> generate -> deliver, as a one-shot call or
a 5-phase guided flow. generation_gate refuses agent_generator until
agent_analyze.check() passes. Driven by projection_surfaces/authoring.py.
"""
...
async def draft_phase(orch, websocket, user_id: str, draft_id: str) -> Tuple[bool, str]:
    ...
    if phase == "clarify":
        ...
        if out is None:
            # Fail closed: an absent list isn't the same as an all-clear one
...
async def start_session(orch, *, user_id: str, agent_name: str, description: str,
    ...
    # Set at creation, not after — keeps this off server execution
    ...
    if not initialized.applied:  # pragma: no cover
...
def save_artifact(orch, user_id: str, draft_id: str, fields: Dict[str, str]) -> Tuple[bool, str]:
    ...
    if phase == "clarify":
        # Empty answers must not fabricate questions — that's a bypass
...
async def generate_from_session(
    ...
    *,
    ...
    # Without this, codegen can invent tools Analyze never saw
    ...
    if generation_expected_revision is None:  # pragma: no cover
