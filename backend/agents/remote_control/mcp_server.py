#!/usr/bin/env python3
"""MCP server for remote-control-1 — routes tool/call requests to verb functions.

Identical dispatch contract to the other bundled agents (dice_roller/weather/
remote_observe): branches on the ``_ui_components`` envelope and flags an error
response when any top-level component has ``variant == "error"``.
"""
import inspect
import json
import logging
...
logger = logging.getLogger('RemoteControlMCPServer')
