...
.PARAMETER AgentType
Optional agent key to update a single agent. Claude and Codex share AGENTS.md. If omitted, updates all existing agent files (creating AGENTS.md if none exist).
...
function Update-ExistingAgentFile {
    ...
    for ($i=0; $i -lt $lines.Count; $i++) {
        ...
        if ($inTech -and $line -match '^##\s') {
            ...
            # Let the next section header be handled below (including Recent Changes).
            $inTech = $false
        }
...
function Update-SpecificAgent {
    ...
    switch ($Type) {
        'claude'   { Update-AgentFile -TargetFile $AGENTS_FILE   -AgentName 'Claude Code' }
        ...
        'amp'      { Update-AgentFile -TargetFile $AGENTS_FILE   -AgentName 'Amp' }
        'shai'     { Update-AgentFile -TargetFile $SHAI_FILE     -AgentName 'SHAI' }
        'tabnine'  { Update-AgentFile -TargetFile $TABNINE_FILE  -AgentName 'Tabnine CLI' }
        'kiro-cli' { Update-AgentFile -TargetFile $AGENTS_FILE   -AgentName 'Kiro CLI' }
        'agy'      { Update-AgentFile -TargetFile $AGY_FILE      -AgentName 'Antigravity' }
        'bob'      { Update-AgentFile -TargetFile $AGENTS_FILE   -AgentName 'IBM Bob' }
...
function Update-AllExistingAgents {
    ...
    # Claude, Codex, opencode, Amp, Kiro and Bob share one file; update it once.
    if (Test-Path $AGENTS_FILE)   { if (-not (Update-AgentFile -TargetFile $AGENTS_FILE   -AgentName 'Shared agents')) { $ok = $false }; $found = $true }
    ...
    if (-not $found) {
        Write-Info 'No existing agent files found, creating AGENTS.md...'
        if (-not (Update-AgentFile -TargetFile $AGENTS_FILE -AgentName 'Shared agents')) { $ok = $false }
    }
    return $ok
}
