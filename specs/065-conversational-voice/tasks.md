...
- [X] T002 Record the 2026-07-31 lead-developer decision approving the RTC-only replacement, prohibit the Agents closure, define the exact final-closure audit/fingerprint gates in `specs/065-conversational-voice/dependency-approval.md`, and require both T004's completed local artifact inventory and its final owner-reviewed closure approval before merge or distribution
- [ ] T003 Before any requested implementation push, extend the deterministic local collector and candidate evidence producers for voice-worker/LiveKit/config/model/client identities plus backend/tooling/Windows Python, web Istanbul, Android app/core Kover, and Apple xccov coverage inputs in `scripts/prepare_release_evidence.py`, `scripts/check_changed_coverage.py`, `.github/workflows/release-readiness.yml`, and `.github/workflows/ci.yml`; prepare the matching owner-reviewed protected-policy uptake through the existing pinned installation flow without letting candidate code approve itself
- [ ] T004 [P] Replace the rejected Agents closure with exact multi-architecture worker pins for direct `livekit==1.1.14`, `numpy==2.4.6`, `onnxruntime==1.28.0`, and the bounded control-channel client `websockets==17.0.1`; move `livekit-api==1.2.0` to the orchestrator boundary; vendor and verify the exact Silero v6.0 ONNX/MIT artifacts; produce locked runtime/test image targets plus a canonical inventory manifest and separate RTC audit in `backend/requirements.txt`, `backend/voice_agent/requirements.in`, `backend/voice_agent/requirements.lock.txt`, `backend/voice_agent/models/`, `backend/voice_agent/CLOSURE.json`, `Dockerfile.voice`, and `specs/065-conversational-voice/dependency-audit-rtc-only-2026-07-31.md`; then replace the inventory-only posture with DHI-backed signature/SBOM/VEX verification, same-candidate dual-architecture zero-HIGH/CRITICAL evidence, and an owner-reviewed final closure fingerprint before checking this task complete
...
- **Setup (Phase 1)**: Started immediately. T004–T010 require the approval decision in T002.
  ...
  overlapping client/project-file edits must preserve that exact predecessor. T003 must complete before any requested implementation push, but it does not block local
  test-first implementation. T004's exact local pins, inventory, locks, images, and audit are the
  local implementation prerequisite; its DHI-backed final fingerprint remains a non-waivable
  merge/distribution gate and does not turn local tests into release evidence.
...
- **Polish (Phase 9)**: Depends on every story selected for delivery; T179 qualifying staging,
  Spec 060 T120 native trust bootstrap, T180 combined all-language coverage/final pre-push evidence,
  and exact dependency approval are non-waivable before merge.
...
- Setup lock/vendor/validator tasks T004–T010 can proceed in parallel after T001–T002 because they
  touch separate dependency surfaces; T003 proceeds independently and gates a requested push.
...
- After T180: the candidate has complete local/staging evidence, subject to protected CI and review.
...
- After T003, rerun the deterministic local evidence collector against the intended base before
  every requested implementation push; its output remains diagnostic and cannot authorize release.
