...
- [ ] T003 Before the next requested implementation push, use one of the two Constitution 2.9 diagnostic paths without claiming merge/release authority. The ordinary path selects one clean committed candidate, regenerates all ten native coverage reports plus canonical local evidence for that exact SHA, and passes strict `BASE_SHA="$(git rev-parse origin/main)" make prepare-release-evidence`. The bounded PR-151 Darwin bootstrap path requires the PR to be draft/non-mergeable; the clean candidate to contain current `origin/main` and fast-forward the prior remote head; fresh `backend_python`, `voice_worker_python`, `tooling_python`, `javascript`, `android_app`, `android_core`, `ios`, `macos`, and `watchos` inputs; an external handled-exit-2 inventory listing exactly `android_evidence`, `backend_evidence`, `docs_evidence`, `ios_evidence`, `macos_evidence`, `watchos_evidence`, `web_evidence`, `windows_evidence`, and `windows_python` as provider-bound missing; one unedited allowlisted-lead comment bound to the exact repository/PR/feature/branch/base/prior head/candidate/changed paths/inventory/policy commit/passed local commands/report digests/blocker/purpose/expiry; and `verify` plus lease-bound `push` from a separate clean checkout of the exact provider default SHA with distinct, external, initially absent inventory/preflight/receipt paths. Retain those authoritative records outside the candidate; mirror only their content-free identities and digests in `specs/065-conversational-voice/verification.md` when preparing a later successor or final-closure candidate, never by amending the approved SHA. Repeat the complete bootstrap and keep or reopen T003 for every repair SHA until final closure. Explicitly record that `dfea619`, `43fba94`, `2332234`, and `a5daf99` were pushed before this diagnostic gate completed; later evidence cannot retroactively make those pushes compliant
...
- [ ] T180 After Spec 060 T120 closes all four native GitHub trust gaps, require the final exact SHA—before PR 151 leaves draft or merges—to regenerate the complete ten-report set and all canonical provider evidence, pass `BASE_SHA="$(git rev-parse origin/main)" make prepare-release-evidence`, and pass protected `scripts/check_changed_coverage.py --fail-under 90` plus the protected release decision without reusing any input or decision from an earlier bootstrap SHA. Prepare and install the matching owner-reviewed protected-policy uptake through the existing pinned installation flow; verify `.github/workflows/release-readiness.yml` keeps that protected verifier/publisher/native-token/exception-debt authority independent of candidate code and free of a repository-scoped GitHub App or custom broker; and record the combined result, installed identity, review, and evidence digests in `specs/065-conversational-voice/verification.md`. The four prerequisites are a distinct non-self environment reviewer, rollback-compatible protected disposable-tag policy, trusted-only signer-eligible tag creation or an equivalent deployed immutable-workflow verifier binding, and uniquely labeled protected publisher hosts with an independent orphan reaper across cancellation/runner loss/host restart/stale lease
...
- [x] T190 Implement and locally verify the RTC-only worker closure inputs—exact pins and hash locks in `backend/voice_agent/requirements.in`, `backend/voice_agent/requirements.lock.txt`, and the matching test manifests; Silero bytes and provenance in `backend/voice_agent/models/` and `backend/voice_agent/licenses/`; runtime/test targets in `Dockerfile.voice` and `Dockerfile.voice.dockerignore`; canonical inventory in `backend/voice_agent/CLOSURE.json`; and RTC audit guards in `backend/tests/test_voice_worker_closure_065.py` and `backend/tests/test_voice_dependency_supply_chain_065.py`. This is local implementation readiness only and does not set `t004_complete`, approve export, or claim a final distribution fingerprint
...
- **Setup (Phase 1)**: Started immediately. T004–T010 require the approval decision in T002.
  ...
  completed local evidence machinery; T003 uses either the complete ordinary pre-push path or the
  exact Constitution 2.9 draft-only bootstrap path with nine fresh Darwin-local reports and nine
  provider-bound absences. Every repair SHA repeats the selected path. T190's completed exact local pins, inventory,
...
- **Polish (Phase 9)**: Depends on every story selected for delivery. The T181–T188 follow-up
  ...
  gates the next requested push, while T179 qualifying staging, Spec 060 T120 native trust-root
  closure, T180 final-SHA ten-report/canonical-evidence coverage and protected-policy uptake, and T004 exact dependency
...
- T189 establishes the deterministic local evidence collector. Complete T003 against one clean
  committed candidate before the next requested implementation push through either the full
  ordinary path or the exact Constitution 2.9 draft-only bootstrap; every repair SHA requires a
  fresh inventory, unedited approval, verification, and lease-bound push. Both outputs remain
  diagnostic. The final exact SHA still requires all ten reports, canonical evidence, the full
  parser, and protected decisions before leaving draft, merging, or releasing.
