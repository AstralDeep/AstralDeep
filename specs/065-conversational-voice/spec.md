...
The user speaks a request and pauses naturally. AstralDeep determines the end of the turn, shows the eligible final recognized text as the user's message, and submits exactly one ordinary chat turn. The same selected LLM, agents, tools, scopes, confirmation requirements, PHI policies, audit provenance, progress trail, persistence, and workspace behavior apply as if the identical text had been typed. The only non-empty-final exclusion is FR-031's bounded exact-match self-speech suppression, which is an internal acoustic false-positive disposition and never a user request.
...
1. **Given** a final non-empty transcript that is not suppressed as exact recent assistant playback under FR-031, **When** end-of-turn is detected, **Then** exactly one authenticated, correlated normal chat message is submitted with that transcript and current chat context.
...
- **FR-012**: If a recognition backend emits partial text, clients MUST display it only as provisional; the launch batch adapter emits no invented partial text. Clients MUST display the final recognized text as the ordinary user message that is submitted. A final classified as internal `self_speech` under FR-031 MUST be discarded before client publication and is not a user-message final.
- **FR-013**: A final non-empty recognized turn that is not FR-031 `self_speech` MUST auto-submit exactly once through the same authenticated, correlated `chat_message` path used by typed input; partial, empty, failed, cancelled, and internally suppressed self-speech recognition MUST NOT submit.
...
- **FR-031**: Assistant playback MUST NOT be recognized and resubmitted as user speech under supported device/audio routes; echo control, speech-state gating, and deduplication MUST be verified together. Speech actually published by the worker MUST be retained only as a bounded ephemeral digest. A final transcript that normalizes to an exact recent digest and whose VAD began within the bounded post-playout suppression window MUST be abandoned as content-free `self_speech` with no proof, dispatch, user-visible error, or retry action. Because exact echo and an intentional verbatim repetition are not acoustically distinguishable, a genuine identical utterance inside that window is conservatively suppressed; different speech and the same phrase after expiry MUST be admitted, and authenticated explicit barge-in MUST stop playback and reopen capture without waiting for the acoustic-tail timer.
...
- An SC-004 recap trial is eligible only while the exact owned session remains active and foreground,
  assistant speech is enabled, and a non-sensitive committed result becomes available without a
  user-requested mute, end, or disconnect before audible recap start. Those intentional
  user-interruption trials are reported separately rather than silently dropped. A product-caused
  disconnect, timeout, TTS/playout failure, or missed two-second deadline remains an eligible miss;
  mute/end/disconnect that occurs after audible recap start cannot remove the trial from the
  denominator.
...
- **SC-006**: In parity tests, 100% of eligible final transcripts—excluding only an exact recent assistant-playback match suppressed under FR-031—produce exactly one ordinary chat submission and the same authorization/confirmation verdict as the identical typed text; FR-031 suppression produces zero submissions or visible retry outcomes. Reconnect and duplicate-event scenarios produce zero duplicate messages, tool calls, or jobs; a new query during active work produces one durable background transition plus one foreground submission in the same accepted-turn transition without cancelling either turn; and every post-navigation turn routes to the newly visible chat while prior turns remain in their originating chats.
...
- **SC-009**: Failure injection for missing credentials, missing model/voice, denied permission, media failure, ASR failure, TTS failure, user-originated request refusal/cancellation/abandonment, timeout, and network loss yields an honest usable text fallback in 100% of cases, visibly distinguishes request non-start/non-completion from post-result speech failure on every client, and exposes zero secret values or raw upstream bodies. The separate FR-031 acoustic false-positive injection MUST instead prove silent `self_speech` suppression with zero submission, notice, or retry guidance.
