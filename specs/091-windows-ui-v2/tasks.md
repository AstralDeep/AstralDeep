...
- [ ] T003 Establish Python 3.11 test environment and baseline Windows/affected shared tests; record commands in `specs/091-windows-ui-v2/verification.md` (FR-012).
- [ ] T004 [P] Test/implement bounded console and presentation decoding in `components/AstralProjection/windows-client/astral_client/console.py` and `tests/test_console.py` (FR-007, FR-008).
- [ ] T005 Test/implement actual capability aggregation and opt-in in `astral_client/protocol.py`, `protocol_manifest.py`, Deep `backend/orchestrator/native_console.py` and Projection `backend/rote/adapter.py` (FR-007, FR-008).
...
- [ ] T006 [US1] Test shell navigation/scenario behavior in `windows-client/tests/test_console_shell.py`, including malformed/empty catalogs and keyboard/drawer dismissal (FR-001, FR-002).
- [ ] T007 [US1] Implement shared-model sidebar/drawer, History, search/account controls, landing categories/scenarios in `astral_client/console_widgets.py` and `app.py` (FR-001, FR-002).
...
- [ ] T008 [US2] Test conversation/result/selection transitions and owner resets in `windows-client/tests/test_console_shell.py` (FR-003, FR-004, FR-009).
- [ ] T009 [US2] Implement bottom composer, More/Advanced, correlated selection and ordered feed with one live result canvas in `astral_client/app.py` (FR-003, FR-004, FR-009).
...
- [ ] T011 [P] [US3] Test and implement six existing primitive renderers in `astral_client/renderer.py`, `charts.py` and `tests/test_console_primitives.py`, including export, actions, zero/non-finite data and accessible feedback (FR-006).
- [ ] T012 [US3] Implement responsive shared settings/surfaces, theme/typography and packaging in `astral_client/app.py`, `theme.py`, `AstralDeep.spec` with tests (FR-005).
