...
Local tests do not establish deployed behavior. Before merge, run the ordinary applicable full backend/module, image/boot and security gates under X/XI and exercise the metrics endpoint through real configured IAM on a candidate deployment. The owner's subsequent PR-review request authorizes publishing the branch and one Deep PR for code review. It creates no staging/release claim, exemption, protected exception, merge or production authorization. This feature does not alter canonical release-evidence producers, publication workflows or their trust model; no release-evidence bootstrap is being invoked.
