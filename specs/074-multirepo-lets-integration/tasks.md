...
- [ ] T001 Refresh all five product remotes without pruning and record exact live heads, defaults, visibility, workflow state, working-tree state, and immutable source SHAs in `Y:/WORK/MCP/AstralDeep/specs/074-multirepo-lets-integration/execution/baseline.json`
- [ ] T002 Verify the signed LETS `v1.0.10` tag object `5ed575066a0c61a51dc55278fa7412f60772fac7` peels to `82dbe4f5ddf410cc86778784bb612440725ec66d` and record the verification result in `Y:/WORK/MCP/AstralDeep/specs/074-multirepo-lets-integration/execution/baseline.json`
- [ ] T003 Inventory path names and sizes, but not contents, for ignored LETS paper/results, Plane database/logs, Projection dependencies, and Deep Android local signing configuration in `Y:/WORK/MCP/AstralDeep/specs/074-multirepo-lets-integration/execution/local-state-continuity.md`
- [ ] T004 [P] Implement exact-root, expected-branch, reparse-point, and clean-index validation in `Y:/WORK/MCP/AstralDeep/scripts/migration/preflight_074.ps1`
- [ ] T005 [P] Implement a staged-path denylist for credentials, databases, logs, uploads, generated agents, local manuscript sources, and generated evidence in `Y:/WORK/MCP/AstralDeep/scripts/migration/check_staged_paths_074.py`
- [ ] T006 [P] Add regression tests for root validation, reparse-point refusal, and sensitive staged-path refusal in `Y:/WORK/MCP/AstralDeep/scripts/tests/test_migration_guards_074.py`
- [ ] T007 Normalize AstralDeep, LETS, and AstralPrimitives fetch/push origins to their canonical HTTPS URLs in `Y:/WORK/MCP/AstralDeep/.git/config`, `Y:/WORK/MCP/LETS/.git/config`, and `Y:/WORK/MCP/AstralPrimitives/.git/config`, fetch each with `--no-prune`, and record verified heads in `execution/baseline.json`
- [ ] T008 Normalize AstralPlane fetch/push origin to `https://github.com/AstralDeep/AstralPlane.git` in `Y:/WORK/MCP/AstralPlane/.git/config`, fetch with `--no-prune`, and stop if the refreshed `master` differs from the baseline recorded in `execution/baseline.json`
- [ ] T009 Create and push AstralPlane `main` at the refreshed `origin/master`, change the GitHub default to `main`, retain `master` unchanged, and record the verified refs in `Y:/WORK/MCP/AstralDeep/specs/074-multirepo-lets-integration/execution/baseline.json`
- [ ] T010 Normalize AstralProjection origin to `https://github.com/AstralDeep/AstralProjection.git`, fetch with `--no-prune`, create and push `main` at refreshed `origin/master`, change the GitHub default to `main`, retain `master` unchanged, and record verified refs in `Y:/WORK/MCP/AstralDeep/specs/074-multirepo-lets-integration/execution/baseline.json`
- [ ] T011 Create `codex/074-extract-data-plane` from Plane `main` in the new exact path `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074` after proving that the branch and path do not already exist
- [ ] T012 Create `codex/074-extract-projection` from Projection `main` in the new exact path `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074` after proving that the branch and path do not already exist
- [ ] T013 Record the ignored status of `Y:/WORK/MCP/AstralDeep/android-client/keystore.properties` and `local.properties`, retain both originals in place, and prohibit removal of their source directory until the copies in Projection pass ignore and signed-build checks in `execution/local-state-continuity.md`
...
- [ ] T018 [P] Add the case-study evidence schema from the planning contract in `Y:/WORK/MCP/LETS/benchmarks/astraldeep/case-study-evidence.schema.json`
...
- [ ] T023 Create the independently installable AstralProjection package scaffold and package-data declarations in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/pyproject.toml` and `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/__init__.py`
- [ ] T024 Create the independently installable Python 3.11 AstralPlane package scaffold and public API boundary in `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/pyproject.toml` and `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/src/astralplane/__init__.py`
...
- [ ] T033 [P] [US3] Import the immutable Deep source revision's tracked `backend/webrender/**` tree into `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/rendering/` and record every source blob in `migration/provenance.json`
- [ ] T034 [P] [US3] Import the immutable Deep source revision's tracked `backend/rote/**` tree into `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/adaptation/` and record every source blob in `migration/provenance.json`
...
- [ ] T036 [P] [US3] Import the complete tracked Windows client product into `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/windows/` with source blobs recorded in `migration/provenance.json`
- [ ] T037 [P] [US3] Import the complete tracked Android client product into `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/android/`, copy the two ignored local properties from Deep without reading/logging/staging them, verify their ignore disposition, and record tracked source blobs in `migration/provenance.json`
- [ ] T038 [P] [US3] Import the complete tracked Apple client products into `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/apple/` with source blobs recorded in `migration/provenance.json`
...
- [ ] T040 [US3] Complete canonical extraction provenance, legacy default baseline, selected paths/blob IDs, and manifest SHA-256 in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/migration/provenance.json`
- [ ] T041 [US3] Replace copied relative imports with stable public package imports and exports in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/__init__.py`
- [ ] T042 [US3] Package templates, JavaScript, CSS, fonts, images, vendor bundles, checksums, and notices through resource accessors in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/resources.py`
- [ ] T043 [P] [US3] Add resource-presence, digest, traversal-refusal, and wheel-install tests in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/tests/test_resources.py`
- [ ] T044 [US3] Define component, frame, chrome, theme, layout, degradation, and device-capability view models without Deep implementation types in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/models.py`
- [ ] T045 [US3] Refactor audit, feedback, onboarding, and admin surfaces into pure supplied-state view builders in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/chrome/admin.py`
- [ ] T046 [US3] Refactor agent, authoring, draft, and attachment surfaces into pure supplied-state view builders in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/chrome/agents.py`
- [ ] T047 [US3] Refactor LLM, personalization, dreaming, pulse, and scheduler surfaces into pure supplied-state view builders in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/chrome/personalization.py`
- [ ] T048 [US3] Refactor remote-machine, feature-flag, workspace, history, and timeline surfaces into pure supplied-state view builders in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/chrome/workspace.py`
- [ ] T049 [P] [US3] Add golden, empty-state, denial-state, sanitization, accessibility, theme, layout, and degradation tests for the pure view builders in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/tests/chrome/`
- [ ] T050 [US3] Add Deep-owned authorized query/command controllers that construct Projection view models in `Y:/WORK/MCP/AstralDeep/backend/orchestrator/projection_controllers.py`
- [ ] T051 [P] [US3] Add controller authorization, owner-isolation, failure, and redaction tests in `Y:/WORK/MCP/AstralDeep/backend/tests/test_projection_controllers.py`
- [ ] T052 [US3] Publish UI protocol version/digest metadata and stable manifest access in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/src/astralprojection/protocol.py`
- [ ] T053 [P] [US3] Repoint Windows protocol/runtime drift tests to `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/contracts/ui_protocol.json` in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/windows/tests/`
- [ ] T054 [P] [US3] Repoint Android protocol/runtime drift tests to the standalone Projection contract in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/android/`
- [ ] T055 [P] [US3] Repoint Apple protocol/runtime drift tests to the standalone Projection contract and remove the missing-manifest skip in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/apple/AstralCore/Tests/`
- [ ] T056 [US3] Rewrite Projection-owned workflow paths and repository identities while leaving copied CI/release workflows inactive in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/workflows-disabled/`
- [ ] T057 [US3] Preserve Android application ID, redirect URI, version-code monotonicity, and signing-key expectations in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/android/app/build.gradle.kts` and `docs/android-release-continuity.md`
- [ ] T058 [US3] Preserve Apple bundle IDs/team/profile expectations and add a protected monotonic build-number base in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/scripts/apple_build_number.py`
- [ ] T059 [P] [US3] Add Apple build-number non-regression and missing-offset refusal tests in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/tests/release/test_apple_build_number.py`
- [ ] T060 [US3] Implement the bounded Windows legacy/new-repository updater trust model in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/windows/astral_client/integrity.py`
- [ ] T061 [P] [US3] Add Windows updater tests for legacy bridge maximum, Projection workflow identity, wrong repository/workflow, downgrade, and identical-bridge-byte requirements in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/windows/tests/test_integrity.py`
- [ ] T062 [US3] Document the non-publishing dual-signature bridge procedure and stable product/store identities in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/docs/release-trust-transition.md`
- [ ] T063 [US3] Run Projection package, renderer, adaptation, architecture, resource, and shared-protocol fixture tests and save command/output digests in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/migration/checks.json`
- [ ] T064 [US3] Commit the exact paths under `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074` as an ordinary descendant with `Source-Repository`, `Source-Commit`, and `Source-Manifest-SHA256` trailers, push `codex/074-extract-projection`, and verify its remote SHA
...
- [ ] T067 [US4] Import only the immutable Deep revision's selected durable-state source and tests into `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/src/astralplane/` and `tests/`, recording source blobs in `migration/provenance.json`
- [ ] T068 [US4] Define public transaction, query, command-result, schema, repository, blob, outbox, lifecycle, and recovery protocols in `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/src/astralplane/contracts.py`
- [ ] T069 [US4] Implement explicit pool/connection/transaction scopes and detached immutable command metadata in `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/src/astralplane/database.py`
...
- [ ] T071 [US4] Replace lexical `?`/`%` SQL rewriting with native psycopg parameter contracts in `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/src/astralplane/sql.py`
...
- [ ] T073 [US4] Implement one explicit, concurrency-safe boot initializer instead of running schema initialization on every `Database()` construction in `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/src/astralplane/bootstrap.py`
...
- [ ] T075 [US4] Implement one guarded repeat-safe schema runner and declared `DataPlaneRevision` in `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/src/astralplane/schema.py`
...
- [ ] T097 [US4] Define schema/API/minimum-consumer compatibility metadata in `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/astralplane-compatibility.json`
...
- [ ] T101 [US4] Run upgrade, repeat-upgrade, transactional failure, blob failure, and documented recovery tests against the representative fixtures and save digests in `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/migration/checks.json`
- [ ] T102 [US4] Complete canonical extraction provenance, legacy default baseline, selected paths/blob IDs, and manifest SHA-256 in `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/migration/provenance.json`
...
- [ ] T113 [US1] Create the authoritative compatible revision, contract version, protocol digest, Plane revision, LETS profile, and availability-mode declaration in `Y:/WORK/MCP/AstralDeep/system-composition.json`
...
- [ ] T118 [P] [US1] Include component sources while continuing to exclude secrets/runtime state in `Y:/WORK/MCP/AstralDeep/.dockerignore`
- [ ] T119 [US1] Update local bootstrap, sync, and preflight targets for submodule initialization and composition validation in `Y:/WORK/MCP/AstralDeep/Makefile`
...
- [ ] T124 [US1] Remove Projection-owned tracked source trees from `Y:/WORK/MCP/AstralDeep/backend/webrender`, `backend/rote`, `windows-client`, `android-client`, and `apple-clients` only after all runtime/build/test references use the submodule
...
- [ ] T138 [US2] Add guarded repeat-safe authority binding/operation/claim/outbox migrations and bump the declared Plane revision in `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074/src/astralplane/schema.py`
...
- [ ] T167 [US2] Add the same verifier/replay/host-binding contract to the supported Windows BYO runtime in `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/windows/win_agent/agent.py`
...
- [ ] T179 [US2] Record the explicit primitive-coverage decision and leave AstralPrimitives unchanged unless `verify_primitive_coverage.py` proves a real vocabulary gap in `Y:/WORK/MCP/AstralDeep/specs/074-multirepo-lets-integration/execution/primitive-decision.json`
...
- [ ] T181 [US2] Commit and push the Plane authority checkpoint on `codex/074-extract-data-plane`, verify its remote SHA/ancestry, and update `Y:/WORK/MCP/AstralDeep/system-composition.json` to that exact commit
- [ ] T182 [US2] Commit and push the Projection BYO verifier checkpoint on `codex/074-extract-projection`, verify its remote SHA/ancestry, and update `Y:/WORK/MCP/AstralDeep/system-composition.json` to that exact commit
...
- [ ] T185 [P] [US5] Replace current operational old/case-mismatched repository URLs in `Y:/WORK/MCP/AstralDeep` with canonical AstralDeep, AstralProjection, AstralPlane, AstralPrimitives, and LETS URLs while preserving explicitly historical citations
...
- [ ] T198 [US5] Produce one exact five-repository `MigrationCheckpoint` plus manifest digest in `Y:/WORK/MCP/AstralDeep/migration/checkpoint-074.json`
- [ ] T199 [US5] Run provenance, ancestry, URL, retained-master, release-trust, source-ownership, and sensitive-path checks and save digests in `Y:/WORK/MCP/AstralDeep/specs/074-multirepo-lets-integration/execution/us5-checks.json`
- [ ] T200 [US5] Commit and push URL/provenance changes under `Y:/WORK/MCP/LETS`, `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074`, `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074`, `Y:/WORK/MCP/AstralPrimitives`, and `Y:/WORK/MCP/AstralDeep` to their named feature branches without changing default-branch product content or publishing a release
...
- [ ] T203 [P] [US6] Implement a tracked case-study runner for off, shadow, and enforce modes across six scopes, lifecycle, parallel/recursive dispatch, outage, replay, exhaustion, and revocation in `Y:/WORK/MCP/LETS/benchmarks/astraldeep/run_case_study.py`
...
- [ ] T216 [US6] Update the anonymous manuscript with neutral self-citation and no prohibited names, organizations, repository URLs, public fingerprints, or deanonymizing acknowledgments in `Y:/WORK/MCP/LETS/paper/submission/anonymous.tex`
...
- [ ] T224 [P] Run Projection Windows tests with `QT_QPA_PLATFORM=offscreen` from `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/windows/`
- [ ] T225 [P] Run Projection Android `ktlintCheck`, lint, core/app unit coverage, and assemble gates from `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/android/gradlew`
- [ ] T226 [P] Run Projection Apple Swift/Xcode gates on a qualified macOS host from `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074/clients/apple/README.md`, or record the exact unavailable-host blocker without weakening the gate
...
- [ ] T233 Recompute every component contract, provenance, source-owner, case-study, and system-composition digest and update `Y:/WORK/MCP/AstralDeep/system-composition.json` and `migration/checkpoint-074.json` only when the exact pins match tested commits
...
- [ ] T236 Commit and push final qualified changes under `Y:/WORK/MCP/.migration-worktrees/AstralPlane-074`, `Y:/WORK/MCP/.migration-worktrees/AstralProjection-074`, `Y:/WORK/MCP/AstralPrimitives`, `Y:/WORK/MCP/LETS`, and `Y:/WORK/MCP/AstralDeep` in dependency order, verifying every remote SHA before advancing the composition
...
- [ ] T238 Keep production on the monorepo-derived deployment and keep hosted/release workflows inactive until a separate owner decision; record the verified posture in `Y:/WORK/MCP/AstralDeep/specs/074-multirepo-lets-integration/execution/delivery-boundary.json`
