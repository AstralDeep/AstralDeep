...
- **FR-011** The surface gains a **Skills** section: list (name, `/command`, applies-to, enabled), create/edit form, enable/disable and delete actions (`chrome_skill_save`, `chrome_skill_toggle`, `chrome_skill_delete`). Web `render()` and native `components()` both.
