...
| Surface | `projection_surfaces/authoring.py` | Home view (web + native): status · quick create · run cards · agents · editor sessions · skills · advanced form. Step editor: Clarify naming, stale-pass warning, Generate replaced by Re-run Analyze when stale. New handlers `chrome_author_quick_{create,answers,resend,dismiss}`, `chrome_skill_{save,edit,toggle,delete}`. Root carries `data-astral-commands`. |
