...
- **FR-036**: A worker whose speech-endpoint preflight failed MUST re-check on a bounded interval so that speech-service recovery (models/routes appearing) restores voice availability without container restarts; each re-check verdict is logged.
  - *FR-045 (065) carve-out, recorded here for conformance audits*: the worker MAY hold a bounded in-process cache of synthesized PCM for the closed, server-owned announcement phrase vocabulary only (`voice_coordinator.APPROVED_PHRASE_TEXT`, drift-pinned by test). User text, transcripts, and result recaps are structurally excluded by exact-match against that vocabulary plus a length ceiling, so 065's zero-retention posture for user content is unchanged; the deviation is limited to operator-authored fixed phrases and exists to meet the 1.5 s acknowledgement budget without a per-turn TTS round trip.
