...
- [X] T004 Add `perf_span` markers `turn.first_llm_call_start` (just before round-1 `_call_llm`) and `turn.first_tool_dispatch` in `D:backend/orchestrator/orchestrator.py` (the latter is a no-op marker). Capture baseline p50/p95 over 200 scripted-LLM turns and 50 real-provider turns on the routing fixture prompts, measuring Send→thinking, Send→first LLM call and Send→first tool dispatch. Record the results in verification.md (SC-001/SC-002 baseline). **Done 2026-09-18.** Both markers fire end to end on real turns through the web client and the socket driver (verification.md 8.16, 8.13). The scripted-LLM half is the seam harness — 200 turns per case against the fake at the distribution measured on the real service (8.16); the real-provider half is 203 keyed turns against the live endpoint. Send→first progress frame is **9.6 ms p50** on the `operation_status` frame emitted at accept; the preparation window and the SC-002 added wait are in 8.16.
...
- [X] T021 [US1] Exercise locally on the web client: save, status, replace, remove. (The native-client half is split out as T021a, which is **not** done.) Record observations in verification.md, and confirm with T002 that no client files changed. **Web half done 2026-09-18** (8.17), re-walked against an image rebuilt from the fixed tree: status → invalid key rejected → real key `ACTIVE` with `Saved key hidden` and absent from the DOM → removed (chat still works, no first-run dialog, zero TypeSafe calls) → re-saved. **The native-client half is NOT done** — no native client build was available to this session. 089 changes no client code (T002/SC-011: `check_089_scope.py` PASS, zero client-directory changes across five repositories), so the same server-driven surface reaches a native client, but that is an argument rather than an observation and is recorded as one.
- [ ] T021a [US1] Exercise the same TypeSafe credential lifecycle on one existing native client build: save, status, replace, remove. **Blocked, not deferred by choice**: no native client build was available to this session. It needs no code — 089 changes no client file (T002/SC-011: `check_089_scope.py` PASS, zero client-directory changes across all five repositories) and the surface is server-driven, so the same projection reaches a native client. That is an argument, not an observation, and this task exists so the distinction is not lost. Whoever has a build can close it in minutes by walking verification.md §8.17's table on it.
...
- [X] T032 [US3] Qualify latency on the local candidate stack: 200 unkeyed turns (SC-001), 200 keyed turns against TypeSafe (SC-002), and injected-failure runs (SC-003) and circuit-open runs (SC-004). Record the distributions against the T004 baseline in verification.md. **Done 2026-09-18.** SC-001: an unkeyed turn through the real browser makes **zero** TypeSafe calls and logs no `turn.typesafe_start` (8.13, 8.17). SC-002: 203 keyed turns, added wait `max(0, routing − preparation)` paired per turn = **0.0 ms at p50, p95 and max** against a 150 ms bound (8.16). SC-003 and SC-004 were already measured at the seam and are now also observed end to end through the UI (8.13 §4): injected `timeout` 3 attempts in 386 ms, `auth` 1 attempt in 46 ms, and an open circuit costing `duration_ms=0` with no call.
...
- [X] T062 Walk through `quickstart.md` end to end on the local candidate stack and record the results. **Done 2026-09-18** (8.13). All nine sections walked. Four defects came out of walking it: the fault switch was never installed (7k), the section-3 dice prompt asked for a die the catalog has no tool for (7j), 7 of this feature's own routing tests failed on any Compose-started stack (7i), and three files the image does not carry make three test commands fail under `docker exec`. Section 2 step 4 (a native client build) and the fault-plus-LLM-failure row of section 4 are the two parts not fully covered; both are noted in place.
